...
class WaitingListContainer extends Component {
    ...
    render() {
        ...
        return (
            <WaitingList input={input} waitingList={list}
                onChange={this.handleChange} onSubmit={this.handleSubmit}
                onEnter={this.handleEnter} onLeave={this.handleLeave}
            />
        )
    }
}
...
//이런 구조로 하면 나중에 다양한 리덕스 모듈을 적용해야 하는 상황에서 유용하다.
const mapDispatchToProps = dispatch => ({
    WaitingActions: bindActionCreators(waitingAcitons, dispatch),
    //AnotherActions: bindActionCreators(anotherAction, dispatch)
})
